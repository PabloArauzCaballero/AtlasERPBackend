...
import { CreateContractHeaderDto, CreateContractTermDto } from '../../shared/schemas/accounting.schemas';
...
export class ContractsService {
  ...
  async list(user: AuthUser) {
    ...
    return { items: rows.filter((row) => {
      try { this.legalEntityAccessService.assertCanAccessLegalEntity(user, row.legalEntityId); return true; } catch { return false; }
    }), total: rows.length };
  }
  ...
  async update(id: string, input: Record<string, unknown>, user: AuthUser) {
    ...
    if (!contract) throw new NotFoundException({ code: 'CONTRACT_NOT_FOUND', message: 'El contrato no existe.' });
    this.legalEntityAccessService.assertCanAccessLegalEntity(user, contract.legalEntityId);
    const allowed = ['contractNo', 'contractType', 'counterpartyBpId', 'startDate', 'endDate', 'currencyCode', 'status'];
