...
| Docker no podía ejecutar migraciones productivas sin devDependencies    | Imagen final no tenía `tsx`, pero los scripts de migración dependían de él                 | Se compilan scripts a `dist/scripts` y se agrega `db:migrate:prod` (`db:seed:prod` se retiró con las semillas del repo)                                     |
