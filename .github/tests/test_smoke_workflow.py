...
assert names.index('Smoke del backend publicado') > names.index('Deploy and wait for Coolify')
step = next(s for s in steps if s.get('name') == 'Smoke del backend publicado')
...
out.write_text(json.dumps({'docker_compose_domains': json.dumps({'api': {'domain': 'https://api.example.test:3007'}})}))
...
''')
    ...
    for name, extra, expected, url in [
        ('coolify-domain', {}, 0, 'https://api.example.test'),
